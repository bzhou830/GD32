...
NTSTATUS PnpAdd(IN WDFDRIVER  Driver, IN PWDFDEVICE_INIT  DeviceInit)
{
	...
	RtlInitUnicodeString(&DeviceName, L"\\Device\\CY001-0") ;   // 设备名
	RtlInitUnicodeString(&DosDeviceName, L"\\DosDevices\\CY001-0");// 符号链接名
	RtlInitUnicodeString(&RefString, L"CY001-0");// 符号链接名
...
)
{
	...
	for(i = 0; i < NumSettings; i++) {
		Pairs[i].UsbInterface = WdfUsbTargetDeviceGetInterface(UsbDevice, (UCHAR)i);// 设置接口句柄
